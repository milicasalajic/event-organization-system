...
function RestoraniPage() {
    ...
    const [dodavanjeError, setDodavanjeError] = useState('');
    const [deleteError, setDeleteError] = useState('');
    const [deletingId, setDeletingId] = useState(null);
    const [restoranZaBrisanje, setRestoranZaBrisanje] = useState(null);

    const [formData, setFormData] = useState({ //spram dto-a
        naziv: '',
        telefon: '',
        radnoVreme: '',
        adresa: '',
    ...
    const korisnikJson = localStorage.getItem('korisnik'); //ovo je json string

    const korisnik = korisnikJson //pretvori u js objekat
        ? JSON.parse(korisnikJson)
        : null;

    const jeAdministrator = korisnik?.uloga === 'ADMINISTRATOR';
    const jeKlijent = korisnik?.uloga === 'KLIJENT';

    const aktivniRestorani = restorani.filter(
        (restoran) => restoran.status === 'AKTIVNO',
    );

    const neaktivniRestorani = jeAdministrator
        ? restorani.filter(
            (restoran) => restoran.status === 'NEAKTIVNO',
        )
        : [];

    useEffect(() => { //cim se komponenta prikaze izvrsi ovaj kod
    ...
    function handleChange(event) { //pri svakom popunjavanju forme
        const { name, value } = event.target;

        setFormData((prev) => ({
            ...prev, //prethodno uneseno
            [name]: value,
        }));
    }
    ...
    async function handleSubmit(event) {
        ...
        try {
            const noviRestoran = await addRestoran(formData);

            setRestorani((prev) => [ //u prethodnu listu restorana dodajes novi
                ...prev,
                {
                    ...noviRestoran,
                    status: noviRestoran.status ?? 'AKTIVNO',
                },
            ]);
            ...
            setDodavanjeError(error.message);
        } finally {
            setIsSaving(false);
        }
    }

    function handleOtvoriBrisanje(event, restoran) {
        event.stopPropagation();

        setDeleteError('');
        setRestoranZaBrisanje(restoran);
    }
    ...
    async function handlePotvrdiBrisanje() {
        ...
        const restoranId = restoranZaBrisanje.restoranId;
        ...
        <div className="restorani-page">
            ...
            <main className="restorani-container">
                <section className="restorani-izbor">
                    ...
                    {jeAdministrator && prikaziFormu && (
                        ...
                        >
                            <div className="dodaj-restoran-forma-header">
                                ...
                                <p>
                                    Unesite osnovne podatke restorana.
                                </p>
                            </div>
                    ...
                    {aktivniRestorani.length === 0 ? (
                        <p className="restorani-empty">
                            Trenutno nema aktivnih restorana u sistemu.
                        </p>
                    ) : (
                        <div className="restorani-lista">
                            {aktivniRestorani.map((restoran) => (
                                <article
                                ...
                                >
                                    ...
                                    <div className="restoran-info">
                                        <p>
                                            ...
                                            <span>{restoran.adresa}</span>
                                        </p>

                                        <p>
                                            <strong>Telefon</strong>
                                            <span>{restoran.telefon}</span>
                                        </p>
                    ...
                    neaktivniRestorani.length > 0 && (
                        <section className="neaktivni-restorani-sekcija">
                            ...
                            <div className="neaktivni-restorani-lista">
                                {neaktivniRestorani.map((restoran) => (
                                    <div
            ...
            {restoranZaBrisanje && (
                <div className="brisanje-modal-overlay">
                    <div className="brisanje-modal">
                        ...
                        <h2>Brisanje restorana</h2>

                        <p>
                            Da li ste sigurni da želite da obrišete restoran
                            <strong>
                                {' '}
                        ...
                        <span className="brisanje-modal-napomena">
                            Restoran će biti označen kao neaktivan i više neće
                        ...
                        <div className="brisanje-modal-actions">
                            <button
                                ...
                                onClick={handleOdustaniOdBrisanja}
                            >
                                Odustani
                            </button>
